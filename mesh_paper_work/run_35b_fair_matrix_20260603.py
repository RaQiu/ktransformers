...
def aggregate(out_root: Path, results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    md = [
        ...
        "",
        "| label | precision | mode | policy | TP | GE | defer | status | ok/expected | decode tok/s | prefill tok/s | peak GiB | file GiB | hit rate | io_uring GiB | summary |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    for r in rows:
        ok = "" if r["ok_count"] is None else f"{r['ok_count']}/{r['expected_prompt_count']}"
        md.append(
            "| {label} | {precision} | {mode} | {policy} | {tp} | {ge} | {defer} | {status} | {ok} | {dec} | {pre} | {peak} | {file} | {hit} | {read} | {summary} |".format(
                label=r["label"],
                precision=r["precision"],
                mode=r["mode"],
                policy=r["code_version_policy"],
