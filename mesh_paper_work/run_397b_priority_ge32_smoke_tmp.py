...
def launch(mode: str, run_dir: Path, name: str, log_path: Path, expert_stats_path: Path, mesh_cap: int) -> subprocess.Popen:
    ...
    config = {
        "mode": mode,
        "cmd": cmd,
        "env_subset": {k: env[k] for k in sorted(env) if k.startswith("KT_") or k in ("CUDA_VISIBLE_DEVICES", "PYTHONPATH", "PATH")},
        "model": MODEL_PATH,
...
def bench_one(name: str, prompt_obj: dict[str, str], max_tokens: int) -> dict[str, Any]:
    ...
    try:
        with requests.post(
            ...
            json=payload,
            stream=True,
            timeout=max(1800, max_tokens * 30),
        ) as resp:
...
def run_mode(mode: str, prompt_data: dict[str, Any], prompts: list[dict[str, str]], out_root: Path, mesh_cap: int) -> dict[str, Any]:
    ...
    name = f"Q397-fair-{label}-tp4-ge32-defer3"
...
def main() -> int:
    global CUDA, MEMORY_MAX, MEM_FRACTION_STATIC
    global MESH_LAYER_MODE, MESH_GLOBAL_POOL_CAPACITY, MESH_PREFILL_LAYER_WINDOW
    global MESH_EARLY_LAYER_EXPERTS, MESH_DECODE_TRANSITION_SYNC, MESH_DECODE_TRANSITION_ON_COLD_Q1
    global MESH_BOOTSTRAP_PREFETCH
    args = parse_args()
    CUDA = args.cuda
