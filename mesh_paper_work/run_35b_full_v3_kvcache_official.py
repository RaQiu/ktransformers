...
OFFICIAL_REPO_URL = "https://github.com/kvcache-ai/ktransformers.git"
OFFICIAL_COMMIT = "c1cb22311bffb402c80fd3758f07d916cd148948"
OFFICIAL_SGLANG_COMMIT = "51032b71279d9038058563f8d2e758d99b278ef4"
OFFICIAL_WORK_ROOT = "/mnt/data2/tmp/qujing_full_v3"
OFFICIAL_ROOT = f"{OFFICIAL_WORK_ROOT}/ktransformers_kvcache_official_clean_20260606"
...
OFFICIAL_VENV_ROOT = f"{OFFICIAL_WORK_ROOT}/venvs/ktransformers-kvcache-official-full-v3-20260606"
OFFICIAL_VENV = f"{OFFICIAL_VENV_ROOT}/bin"

PROMPT_FILE = Path("/mnt/data3/work/mesh_standard_5domain_prompts_20260602.json")
DEFAULT_OUT_ROOT = Path("/mnt/data3/work/mesh_paper_35b_runs/full_v3_kvcache_official_20260606")
...
def validate_official_checkout_clean() -> dict[str, Any]:
    root = Path(OFFICIAL_ROOT)
    if not root.exists():
        raise FileNotFoundError(f"official checkout does not exist: {root}")
    checks = {
        "repo_url": shell(f"cd {OFFICIAL_ROOT!r} && git remote get-url origin", timeout=15).strip(),
        "commit": shell(f"cd {OFFICIAL_ROOT!r} && git rev-parse HEAD", timeout=15).strip(),
        "sglang_commit": shell(f"cd {OFFICIAL_ROOT!r}/third_party/sglang && git rev-parse HEAD", timeout=15).strip(),
    }
    if checks["repo_url"] != OFFICIAL_REPO_URL:
        raise RuntimeError(f"official repo mismatch: {checks['repo_url']} != {OFFICIAL_REPO_URL}")
    if checks["commit"] != OFFICIAL_COMMIT:
        raise RuntimeError(f"official commit mismatch: {checks['commit']} != {OFFICIAL_COMMIT}")
    if checks["sglang_commit"] != OFFICIAL_SGLANG_COMMIT:
        raise RuntimeError(f"official sglang commit mismatch: {checks['sglang_commit']} != {OFFICIAL_SGLANG_COMMIT}")
    scan = shell(
        "cd {root} && "
        "(test ! -d kt-kernel/operators/mesh) && "
        "(! rg -n 'MESHIO|KT_MESH|KT_MAX_RESIDENT_EXPERTS|file_slots' "
        "kt-kernel ktransformers third_party/sglang/python 2>/dev/null)".format(root=repr(OFFICIAL_ROOT)),
        timeout=30,
    )
    checks["contamination_scan_output"] = scan
    return checks
...
def parse_runtime_log(path: Path) -> dict[str, Any]:
    ...
    contamination_markers = {
        "MESHIO": text.count("MESHIO"),
        "file_slots": text.count("file_slots"),
        "io_uring": text.count("io_uring"),
        "KT_MESH": text.count("KT_MESH"),
        "operators/mesh": text.count("operators/mesh"),
    }
    return {
        ...
        "official_contamination_markers": contamination_markers,
        "official_contamination_count": sum(contamination_markers.values()),
    }


def apply_official_validity_gate(entry: dict[str, Any]) -> None:
    row = entry.get("row_summary") or {}
    mem = entry.get("memory_summary") or {}
    log_summary = entry.get("runtime_log_summary") or {}
    entry["ok_count"] = row.get("ok_count")
    entry["api_decode_tok_s"] = row.get("decode_tok_s_avg_api")
    entry["api_prefill_tok_s"] = row.get("prefill_tok_s_avg_api")
    entry["api_total_tok_s"] = row.get("total_tok_s_avg_api")
    entry["peak_memory_gib"] = mem.get("peak_gib")
    entry["peak_anon_gib"] = mem.get("peak_anon_gib")
    entry["peak_file_gib"] = mem.get("peak_file_gib")
    entry["peak_file_mapped_gib"] = mem.get("peak_file_mapped_gib")
    contamination_count = int(log_summary.get("official_contamination_count") or 0)
    entry["official_contamination_count"] = contamination_count
    entry["official_contamination_markers"] = log_summary.get("official_contamination_markers") or {}
    if contamination_count:
        entry["status"] = "invalid_contaminated"
        entry["fail_tail"] = "Official full-v3 log contains MESH/file-slot/io_uring markers"
        return
    model_text = " ".join(
        str(entry.get(k) or "")
        for k in ("model", "model_path", "kt_weight_path", "label")
    )
    if (
        entry.get("status") == "ready"
        and "397" in model_text
        and mem.get("peak_gib") is not None
        and float(mem["peak_gib"]) < 100.0
    ):
        entry["status"] = "invalid_memory_sanity"
        entry["fail_tail"] = "397B official full-v3 completed with implausibly low cgroup peak memory"
...
def write_matrix_markdown(out_root: Path, plan: list[dict[str, Any]]) -> None:
    lines = [
        "# Official full-v3 benchmark matrix",
        "",
...
def launch_one(run: dict[str, Any], out_root: Path, prompts: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        apply_official_validity_gate(entry)
...
def main() -> int:
    ...
    checkout_check = validate_official_checkout_clean()
    write_json(out_root / "official_checkout_check.json", checkout_check)
