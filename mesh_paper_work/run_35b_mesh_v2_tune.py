...
LEGACY_PY = "/mnt/data3/work/venvs/qr-sglang-clean/bin/python"
LEGACY_KT_LIB = "/mnt/data3/work/ktransformers_mmap_d7b5b49/kt-kernel/build/lib.linux-x86_64-cpython-311"
LEGACY_SGLANG = "/mnt/data3/work/sglang_mmap_d7b5b49_compat"

PROMPT_FILE = Path("/mnt/data3/work/mesh_standard_5domain_prompts_20260602.json")
DEFAULT_OUT_ROOT = Path("/mnt/data3/work/mesh_fair_matrix_35b_20260603")
...
def mode_policy(mode: str) -> str:
    return "legacy_old_checkout" if mode == "mmap" else "current_latest"
...
def build_env(mode: str, precision: str, cuda: str, expert_stats_path: Path, args: argparse.Namespace) -> tuple[str, dict[str, str]]:
    ...
    env.update(
        {
            ...
            "TRITON_CACHE_DIR": str(args.triton_cache_dir),
    ...
    if mode == "mmap":
        env["PATH"] = f"{Path(LEGACY_PY).parent}:" + env.get("PATH", "")
        env["PYTHONPATH"] = f"{LEGACY_SGLANG}:{LEGACY_KT_LIB}" + (":" + env["PYTHONPATH"] if env.get("PYTHONPATH") else "")
        return LEGACY_PY, env

    current_venv_bin = str(args.current_venv_bin)
    env["PATH"] = f"{current_venv_bin}:" + env.get("PATH", "")
    py = f"{current_venv_bin}/python3"
    if mode == "full":
        ...
        mesh_env = {
            ...
            "KT_RESIDENCY_POLICY": args.mesh_policy,
            "KT_MESH_STATE_DEFER": str(args.mesh_state_defer),
            "KT_MESH_DEFER_PREFETCH": str(args.mesh_defer_prefetch),
            "KT_MESH_BOOTSTRAP_PREFETCH": "1" if args.mesh_bootstrap_prefetch else "0",
            "KT_MESH_PREFILL_LAYER_MODE": str(args.mesh_prefill_layer_mode),
            "KT_MESH_PREFILL_LAYER_WINDOW": str(args.mesh_prefill_layer_window),
            "KT_MESH_DECODE_TRANSITION_SYNC": str(args.mesh_decode_transition_sync),
            "KT_MESH_DECODE_TRANSITION_ON_COLD_Q1": str(args.mesh_decode_transition_on_cold_q1),
            "KT_ENABLE_CACHE_STATS": "1",
            "KT_EXPERT_STATS_PATH": str(expert_stats_path),
            "KT_EXPERT_STATS_DUMP_EVERY": str(args.expert_stats_dump_every),
        }
        optional_mesh_env = {
            "KT_MESH_STATE_DEFER_PREFILL": args.mesh_state_defer_prefill,
            "KT_MESH_BOOTSTRAP_PREFETCH_LIMIT": args.mesh_bootstrap_prefetch_limit,
            "KT_MESH_PREFILL_STATIC_EXPERTS": args.mesh_prefill_static_experts,
            "KT_MESH_EARLY_LAYER_EXPERTS": args.mesh_early_layer_experts,
            "KT_MESH_DECODE_TRANSITION_FILL_LIMIT": args.mesh_decode_transition_fill_limit,
            "KT_MESH_DECODE_TRANSITION_IN_FORWARD": args.mesh_decode_transition_in_forward,
            "KT_MESH_FULL_GATE": args.mesh_full_gate,
            "KT_MESH_FULL_GATE_BATCHED": args.mesh_full_gate_batched,
            "KT_MESH_FULL_GATE_MAX_ELEMENTS": args.mesh_full_gate_max_elements,
            "KT_MESH_LOOKAHEAD": args.mesh_lookahead,
            "KT_MESH_LOOKAHEAD_WEIGHT": args.mesh_lookahead_weight,
            "KT_MESH_HEAT_GAMMA": args.mesh_heat_gamma,
            "KT_MESH_HEAT_BETA": args.mesh_heat_beta,
            "KT_MESH_TRANSITION_ALPHA": args.mesh_transition_alpha,
        }
        for key, value in optional_mesh_env.items():
            if value is not None:
                mesh_env[key] = str(value)
        env.update(mesh_env)
        if precision == "BF16":
            env["KT_ENABLE_BF16_WAVE_RESIDENT"] = "1"
            env["KT_MESH_BF16_EXPERT_CACHE"] = str(args.bf16_expert_cache)
            if args.bf16_expert_cache_dir:
                env["KT_MESH_BF16_EXPERT_CACHE_DIR"] = str(args.bf16_expert_cache_dir)
    else:
...
def build_command(
    ...
    if run["mode"] != "mmap" and not args.disable_dynamic_expert_update:
...
def make_plan(args: argparse.Namespace, prompts: list[dict[str, str]]) -> list[dict[str, Any]]:
    modes = ["full" if m == "fill" else m for m in args.modes]
    plan = []
    for precision in args.precisions:
        ...
        for tp in args.tps:
            for mode in modes:
                ...
                if precision == "BF16" and mode == "mmap" and args.bf16_mmap_model_path:
                    spec["model_path"] = args.bf16_mmap_model_path
                    spec["kt_weight_path"] = args.bf16_mmap_kt_weight_path or args.bf16_mmap_model_path
                ...
                plan.append(
                    {
                        ...
                        "defer": args.defer,
                        "mesh_cap": args.mesh_cap if mode == "mesh" else None,
                        "mesh_global_pool_capacity": args.mesh_global_pool_capacity if mode == "mesh" else None,
                        "mesh_prefill_layer_window": args.mesh_prefill_layer_window if mode == "mesh" else None,
                        "mesh_policy": args.mesh_policy if mode == "mesh" else None,
                        "mesh_state_defer": args.mesh_state_defer if mode == "mesh" else None,
                        "mesh_defer_prefetch": args.mesh_defer_prefetch if mode == "mesh" else None,
                        "mesh_state_defer_prefill": args.mesh_state_defer_prefill if mode == "mesh" else None,
                        "mesh_prefill_layer_mode": args.mesh_prefill_layer_mode if mode == "mesh" else None,
                        "mesh_prefill_static_experts": args.mesh_prefill_static_experts if mode == "mesh" else None,
                        "mesh_early_layer_experts": args.mesh_early_layer_experts if mode == "mesh" else None,
                        "mesh_decode_transition_sync": args.mesh_decode_transition_sync if mode == "mesh" else None,
                        "mesh_decode_transition_on_cold_q1": args.mesh_decode_transition_on_cold_q1
                        if mode == "mesh"
                        else None,
                        "mesh_decode_transition_fill_limit": args.mesh_decode_transition_fill_limit
                        if mode == "mesh"
                        else None,
                        "mesh_decode_transition_in_forward": args.mesh_decode_transition_in_forward
                        if mode == "mesh"
                        else None,
                        "mesh_full_gate": args.mesh_full_gate if mode == "mesh" else None,
                        "mesh_lookahead": args.mesh_lookahead if mode == "mesh" else None,
                        "bf16_expert_cache": args.bf16_expert_cache if precision == "BF16" and mode == "mesh" else None,
                        "bf16_expert_cache_dir": args.bf16_expert_cache_dir
                        if precision == "BF16" and mode == "mesh"
                        else None,
...
def write_matrix_markdown(out_root: Path, plan: list[dict[str, Any]]) -> None:
    lines = [
        "# 35B fair benchmark matrix",
        "",
        "Policy: full/fill and mesh use current latest code; mmap uses the old isolated pre-MESH checkout.",
        "",
    ...
    for r in plan:
        runner = "legacy mmap python" if r["mode"] == "mmap" else "current python"
        lines.append(
...
def write_run_record(run_dir: Path, entry: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- MESH state_defer/defer_prefetch/prefill_state_defer: {entry.get('mesh_state_defer')}/{entry.get('mesh_defer_prefetch')}/{entry.get('mesh_state_defer_prefill')}",
        f"- MESH prefill_layer_mode/static/early_layers: {entry.get('mesh_prefill_layer_mode')}/{entry.get('mesh_prefill_static_experts')}/{entry.get('mesh_early_layer_experts')}",
        f"- MESH decode_transition sync/cold_q1/fill_limit/in_forward: {entry.get('mesh_decode_transition_sync')}/{entry.get('mesh_decode_transition_on_cold_q1')}/{entry.get('mesh_decode_transition_fill_limit')}/{entry.get('mesh_decode_transition_in_forward')}",
        f"- MESH full_gate/lookahead: {entry.get('mesh_full_gate')}/{entry.get('mesh_lookahead')}",
...
def launch_one(run: dict[str, Any], out_root: Path, prompts: list[dict[str, str]], args: argparse.Namespace) -> dict[str, Any]:
    port = int(args.port)
    cuda = args.cuda.strip() if args.cuda.strip() else choose_gpus(int(run["tp"]), args.min_gpu_free_mib, args.wait_gpu_s)
    if len([part for part in cuda.split(",") if part.strip()]) != int(run["tp"]):
        raise ValueError(f"--cuda must contain exactly TP={run['tp']} GPU ids, got {cuda!r}")
    run_dir = out_root / run["label"]
    run_dir.mkdir(parents=True, exist_ok=True)
    name = f"mesh-fair-35b-{run['precision'].lower()}-{run['mode']}-tp{run['tp']}-{now_stamp()}"
    ...
    entry: dict[str, Any] = {
        **run,
        ...
        "env_subset": {
            k: env[k]
            for k in sorted(env)
            if k.startswith("KT_") or k in ("CUDA_VISIBLE_DEVICES", "PYTHONPATH", "PATH", "TMPDIR", "HF_HOME", "TRANSFORMERS_CACHE", "TRITON_CACHE_DIR")
        },
...
def aggregate(out_root: Path, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        rows.append(
            {
                ...
                "mesh_state_defer": r.get("mesh_state_defer"),
                "mesh_defer_prefetch": r.get("mesh_defer_prefetch"),
                "mesh_decode_transition_on_cold_q1": r.get("mesh_decode_transition_on_cold_q1"),
                "mesh_full_gate": r.get("mesh_full_gate"),
                "mesh_lookahead": r.get("mesh_lookahead"),
    ...
    md = [
        "# 35B fair matrix aggregate",
        "",
        "Effective rows require status=ready and ok_count=expected_prompt_count.",
        "",
        "| label | precision | mode | policy | TP | GE | defer | state_defer | cold_q1 | full_gate | lookahead | status | ok/expected | decode tok/s | prefill tok/s | peak GiB | file GiB | hit rate | io_uring GiB | summary |",
        "| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    for r in rows:
        ok = "" if r["ok_count"] is None else f"{r['ok_count']}/{r['expected_prompt_count']}"
        md.append(
            "| {label} | {precision} | {mode} | {policy} | {tp} | {ge} | {defer} | {state_defer} | {cold_q1} | {full_gate} | {lookahead} | {status} | {ok} | {dec} | {pre} | {peak} | {file} | {hit} | {read} | {summary} |".format(
                label=r["label"],
                precision=r["precision"],
                mode=r["mode"],
                policy=r["code_version_policy"],
                ...
                ge=r["gpu_experts"],
                defer=r["defer"],
                state_defer="" if r["mesh_state_defer"] is None else r["mesh_state_defer"],
                cold_q1="" if r["mesh_decode_transition_on_cold_q1"] is None else r["mesh_decode_transition_on_cold_q1"],
                full_gate="" if r["mesh_full_gate"] is None else r["mesh_full_gate"],
                lookahead="" if r["mesh_lookahead"] is None else r["mesh_lookahead"],
                status=r["status"],
                ok=ok,
                ...
                hit=f"{r['hit_rate']:.4f}" if r["hit_rate"] is not None else "",
                read=f"{r['iouring_read_gib']:.3f}" if r["iouring_read_gib"] is not None else "",
...
def main() -> int:
    ...
    parser.add_argument("--precisions", nargs="+", default=["AMXINT4", "BF16"])
    parser.add_argument("--modes", nargs="+", default=["full", "mmap", "mesh"])
    ...
    parser.add_argument("--bf16-mmap-model-path", default="")
    parser.add_argument("--bf16-mmap-kt-weight-path", default="")
    parser.add_argument("--only", nargs="*", default=[])
    parser.add_argument("--port", type=int, default=31850)
    parser.add_argument("--current-venv-bin", default=CURRENT_VENV)
    parser.add_argument("--cuda", default="", help="Comma-separated GPU ids. Defaults to automatic free-GPU selection.")
    parser.add_argument("--tmpdir", type=Path, default=Path("/mnt/data3/work/tmp"))
    parser.add_argument("--triton-cache-dir", type=Path, default=Path("/home/qujing/.triton/cache"))
    ...
    parser.add_argument("--mesh-cap", type=int, default=128)
    parser.add_argument("--mesh-global-pool-capacity", type=int, default=5120)
    ...
    parser.add_argument("--mesh-bootstrap-prefetch-limit", type=int, default=None)
    parser.add_argument("--mesh-state-defer", choices=["0", "1"], default="1")
    parser.add_argument("--mesh-defer-prefetch", choices=["0", "1"], default="1")
    parser.add_argument("--mesh-state-defer-prefill", choices=["0", "1"], default=None)
    parser.add_argument("--mesh-prefill-layer-mode", choices=["0", "1"], default="1")
    parser.add_argument("--mesh-prefill-static-experts", default=None)
    parser.add_argument("--mesh-early-layer-experts", default=None)
    parser.add_argument("--mesh-decode-transition-sync", choices=["0", "1"], default="0")
    parser.add_argument("--mesh-decode-transition-on-cold-q1", choices=["0", "1"], default="0")
    parser.add_argument("--mesh-decode-transition-fill-limit", type=int, default=None)
    parser.add_argument("--mesh-decode-transition-in-forward", choices=["0", "1"], default=None)
    parser.add_argument("--mesh-full-gate", choices=["0", "1"], default=None)
    parser.add_argument("--mesh-full-gate-batched", choices=["0", "1"], default=None)
    parser.add_argument("--mesh-full-gate-max-elements", type=int, default=None)
    parser.add_argument("--mesh-lookahead", choices=["0", "1"], default=None)
    parser.add_argument("--mesh-lookahead-weight", default=None)
    parser.add_argument("--mesh-heat-gamma", default=None)
    parser.add_argument("--mesh-heat-beta", default=None)
    parser.add_argument("--mesh-transition-alpha", default=None)
    parser.add_argument("--expert-stats-dump-every", type=int, default=128)
